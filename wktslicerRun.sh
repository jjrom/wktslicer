...
SELECT count(identifier) FROM inputwkts WHERE ST_intersects(footprint, GeomFromText('POLYGON((1.1013793945311965 43.52751299421623,1.7605590820312442 43.52751299421623,1.7605590820312442 43.68636569542979,1.1013793945311965 43.68636569542979,1.1013793945311965 43.52751299421623))', 4326));
EOF

psql -U postgres -d wktslicer << EOF
\timing on
-- Indirect query using slicing
SELECT count(distinct(i_identifier)) FROM outputwkts WHERE ST_intersects(footprint, GeomFromText('POLYGON((1.1013793945311965 43.52751299421623,1.7605590820312442 43.52751299421623,1.7605590820312442 43.68636569542979,1.1013793945311965 43.68636569542979,1.1013793945311965 43.52751299421623))', 4326));
EOF
...
\timing on
-- Direct query on inputwkts
SELECT count(identifier) FROM inputwkts WHERE ST_intersects(footprint, GeomFromText('POLYGON((-9 44, 4 42,-2 37,-10 36,-9 44))', 4326));
EOF

psql -U postgres -d wktslicer << EOF
\timing on
-- Indirect query using slicing
SELECT count(distinct(i_identifier)) FROM outputwkts WHERE ST_intersects(footprint, GeomFromText('POLYGON((-9 44, 4 42,-2 37,-10 36,-9 44))', 4326));
EOF
